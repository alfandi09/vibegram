...
const RegWizard = new Wizard('reg_wizard', [
    ...
    async (ctx) => {
        ...
        await ctx.reply(`Wizard complete!\nName: ${ctx.wizard!.state.name}\nCity: ${ctx.wizard!.state.city}`);
        ctx.wizard?.leave();
    }
]);
...
// 5. MEDIA
// ==========================================
bot.command('photo', ctx => ctx.replyWithPhoto('https://picsum.photos/400', { caption: 'Photo via URL.' }));
bot.command('document', ctx => ctx.replyWithDocument('https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf', { caption: 'PDF document.' }));
bot.command('audio', ctx => ctx.replyWithAudio('https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3', { caption: 'Audio file.' }));
bot.command('video', ctx => ctx.replyWithVideo('http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerJoyrides.mp4', { caption: 'Video file.' }));
bot.command('voice', ctx => ctx.replyWithVoice('https://upload.wikimedia.org/wikipedia/commons/4/47/PNG_transparency_demonstration_2.ogg'));
bot.command('videonote', ctx => ctx.reply('Video notes require a pre-uploaded circular video file. Send one to the bot to test the on(video_note) handler.'));
bot.command('sticker', ctx => ctx.replyWithSticker('CAACAgIAAxkBAAEDcm1hY0TpAAGVMqU5QjZRzkQxsqx6jyMAAgEAA8A2TxP0bFmGlVVlEiEE'));
bot.command('animation', ctx => ctx.replyWithAnimation('https://media.giphy.com/media/ICOgUNjpvO0PC/giphy.gif', { caption: 'GIF animation.' }));
...
bot.command('quiz', async (ctx) => {
    await ctx.replyWithPoll('What does HTML stand for?', [
...
bot.command('dice', async (ctx) => await ctx.replyWithDice({ emoji: '🎰' }));
bot.command('location', async (ctx) => await ctx.replyWithLocation(-6.2088, 106.8456));
bot.command('venue', async (ctx) => await ctx.replyWithVenue(-6.2088, 106.8456, 'National Monument', 'Central Jakarta'));
...
bot.command('edit_caption', async (ctx) => {
    const msg = await ctx.replyWithPhoto('https://picsum.photos/400', { caption: 'Original caption.' });
    setTimeout(() => {
...
bot.command('edit_markup', async (ctx) => {
    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('Before Edit', 'noop')]
    ]);
...
// 8. FORMATTING
// ==========================================
bot.command('html', ctx => ctx.replyWithHTML('<b>Bold</b>, <i>Italic</i>, <code>Monospace</code>, <a href="https://telegram.org">Link</a>'));
bot.command('markdown', ctx => ctx.replyWithMarkdown('*Bold*, _Italic_, `Monospace`, [Link](https://telegram.org)'));
bot.command('markdownv2', ctx => ctx.replyWithMarkdownV2('*Bold*, _Italic_, `Monospace`, [Link](https://telegram\\.org)'));
...
bot.command('paid_media', async (ctx) => {
    await ctx.replyWithPaidMedia(15, [
...
bot.command('mute_test', async (ctx) => {
    await ctx.reply('restrictChatMember simulation — requires Supergroup context and admin privileges.');
});

bot.command('promote_test', async (ctx) => {
    await ctx.reply('promoteChatMember simulation — requires Supergroup context and admin privileges.');
});
...
bot.command('inline_keyboard', async (ctx) => {
    ...
    await ctx.reply('Inline keyboard with callbacks, URLs, and inline queries:', { reply_markup: keyboard });
});
...
// 12. PAGINATION
// ==========================================
const sampleData = [...Array(15)].map((_, i) => ({ text: `Item ${i + 1}`, callback_data: `p_id_${i + 1}` }));

bot.command('pagination', async (ctx) => {
    const layout = Markup.pagination(sampleData, {
        currentPage: 1, itemsPerPage: 4, columns: 2,
        actionNext: 'np.next', actionPrev: 'np.prev'
    });
...
bot.action(/np\.(next|prev)/, async (ctx) => {
    ...
    const layout = Markup.pagination(sampleData, {
        currentPage: ctx.session.cursor, itemsPerPage: 4, columns: 2,
        actionNext: 'np.next', actionPrev: 'np.prev'
    });
    await ctx.answerCbQuery();
    await bot.callApi('editMessageReplyMarkup', {
        ...
        reply_markup: layout.inline_keyboard
    }).catch(() => {});
});
