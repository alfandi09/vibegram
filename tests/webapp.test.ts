...
function generateInitData(data: Record<string, string>, token: string, overwrite?: Partial<{ hash: string; auth_date: number }>): string {
    ...
    const hash = overwrite?.hash ?? crypto.createHmac('sha256', secretKey).update(dataCheckString).digest('hex');
...
describe('WebAppUtils.validate()', () => {
    it('validates correct initData successfully', () => {
        const initData = generateInitData({ user: JSON.stringify({ id: 1, first_name: 'Test' }) }, TOKEN);
    ...
    it('throws when hash is missing', () => {
        ...
        expect(() => WebAppUtils.validate(TOKEN, params.toString())).toThrow('Hash signature not found');
    });
    ...
    it('throws when auth_date is expired', () => {
        ...
        expect(() => WebAppUtils.validate(TOKEN, initData, { maxAgeSeconds: 86400 })).toThrow('expired');
    });
