...
5. `maxJsonPayloadBytes` limits outgoing JSON payloads, while `maxResponseBytes` limits incoming JSON/text/buffer responses. Oversized API responses fail as `NetworkError` with the bot token redacted.
